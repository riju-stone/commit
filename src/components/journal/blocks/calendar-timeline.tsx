...
const DayItem = memo(({ day }: { day: Date }) => {
  ...
  return (
    <motion.button
      ...
      whileHover="hover"
    >
      <motion.div className="h-px bg-white/40" variants={DAY_ITEM_LINE} />
      <motion.div className="text-sm text-white/40" variants={DAY_ITEM_LABEL}>
        {dayLabel}
      </motion.div>
    </motion.button>
  );
    ...
    <div className="w-full h-full flex flex-col px-4 overflow-y-auto py-2">
      ...
      <div className="w-full flex flex-col gap-4">
        {sortedMonthsWithLabels.map(({ month, label, days }) => (
          <MonthSection key={`month-${month}`} month={month} days={days} monthLabel={label} />
        ))}
      </div>
    </div>
  );
