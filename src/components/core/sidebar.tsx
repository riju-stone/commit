...
function SidebarComponent({ isOpen, children }: SidebarComponentProps) {
  return (
    <motion.div
      layout
      className="shrink-0 h-screen bg-black/50 backdrop-blur-sm flex flex-col items-start justify-start"
      variants={SIDEBAR_CONTENT_ANIMATION.sidebarWrapper}
      initial="closed"
      animate={isOpen ? "open" : "closed"}
      transition={SIDEBAR_CONTENT_ANIMATION.transition}
    >
      {children && <div className="w-full h-full overflow-x-hidden text-nowrap">{children}</div>}
    </motion.div>
  );
}
