...
export function generateMonthView(month: number, year: number, weekStartDay: WeekStartDay = 0): CalendarMonthView {
  ...
  const calendarEnd = endOfWeek(addWeeks(calendarStart, WEEKS_IN_MONTH_VIEW - 1), { weekStartsOn: weekStartDay });
...
export function navigateDate(currentDate: Date, direction: "prev" | "next", unit: "month" | "week" | "day"): Date {
  const navigators = {
